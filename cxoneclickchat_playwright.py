...
def oneclickchat_test(playwright):
    result = 'PASSED'

    time_stamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    har_filename1 = f'oneclickchat_staff_{time_stamp}.zip'
    har_filename2 = f'oneclickchat_visitor_{time_stamp}.zip'
    ...
    try:
        ...
        result = 'FAILED'
        lgr.error(str(e))
        browser1.page.screenshot(path=f'screens/oneclickchat_s1_{time_stamp}.png')
        browser2.page.screenshot(path=f'screens/oneclickchat_s2_{time_stamp}.png')
        ...
        if result == 'FAILED':
            try:
                ...
                notification = f'Monitor - One-click Chat Test Failed'
                notification += f'\nHAR file:\n\t{har_filename1}\n\t{har_filename2}'
                notification += f'\nVideo file:\n\t{video1}\n\t{video2}'
...
if __name__ == '__main__':
    ...
    config = yaml.load(open('config.yaml'), Loader=yaml.FullLoader)['one_click_chat']
